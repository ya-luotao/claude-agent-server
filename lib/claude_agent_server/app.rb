...
module ClaudeAgentServer
  class App < Roda
    ...
    route do |r| # rubocop:disable Metrics/BlockLength
      ...
      r.on 'query' do
        ...
        r.on 'stream' do
          r.post do
            ...
            r.halt [200, response.headers, Services::SseStream.stream_query(prompt: prompt, options: options)]
          end
        end
      ...
      # /cli-sessions routes (read-only CLI session browsing)
      ...
      r.on 'sessions' do # rubocop:disable Metrics/BlockLength
        ...
        r.is do
          ...
          r.post do
            ...
            entry = manager.create_session(options: options, prompt: prompt)
            response.status = 201
        ...
        r.on String do |session_id| # rubocop:disable Metrics/BlockLength
          ...
          r.on 'messages' do
            ...
            r.on 'stream' do
              r.get do
                ...
                response['cache-control'] = 'no-cache'
                response['x-accel-buffering'] = 'no'
                r.halt [200, response.headers, Services::SseStream.stream_session(entry)]
              end
            end
          ...
          r.on 'history' do
            r.get do
              entry = manager.get_session(session_id)
              messages = entry.messages.map { |m| Services::MessageSerializer.serialize(m) }
              json_response({ sessionId: session_id, messages: messages })
            end
          end
        end
      end
